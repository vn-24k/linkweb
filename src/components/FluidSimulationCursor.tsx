...
interface FluidSimulationCursorProps {
...
export default function FluidSimulationCursor({
  simResolution = 128,
  dyeResolution = 1440,
  ...
  useEffect(() => {
    ...
    function getWebGLContext(canvasEl: HTMLCanvasElement) {
      ...
      let webgl: any = canvasEl.getContext("webgl2", params);

      if (!webgl) {
        webgl = canvasEl.getContext("webgl", params) ||
      ...
      if (isWebGL2) {
        ...
        formatRGBA = getSupportedFormat(webgl, webgl.RGBA, webgl.RGBA, halfFloatTexType, supportLinearFiltering);
        formatRG = getSupportedFormat(webgl, webgl.RGBA, webgl.RGBA, halfFloatTexType, supportLinearFiltering);
        formatR = getSupportedFormat(webgl, webgl.RGBA, webgl.RGBA, halfFloatTexType, supportLinearFiltering);
      }
      ...
      if (!supportRenderTextureFormat(webgl, internalFormat, format, type)) {
        if ("drawBuffers" in webgl) {
          switch (internalFormat) {
            case webgl.R16F:
              return getSupportedFormat(webgl, webgl.RG16F, webgl.RG, type, supportFill);
            case webgl.RG16F:
              return getSupportedFormat(webgl, webgl.RGBA16F, webgl.RGBA, type, supportFill);
            default:
              return null;
          }
        }
    ...
    function step(dt: number) {
      ...
      if (gradienSubtractProgram.uniforms.uVelocity) {
        gl.uniform1i(gradienSubtractProgram.uniforms.uVelocity, velocityFBO.read.attach(1));
      }
      blit(velocityFBO.write);
      velocityFBO.swap();

      // Advection - velocityFBO
      ...
      // Advection - dyeFBO
      if (!ext!.supportLinearFiltering && advectionProgram.uniforms.dyeTexelSize) {
        gl.uniform2f(advectionProgram.uniforms.dyeTexelSize, dyeFBO.texelSizeX, dyeFBO.texelSizeY);
      }
      if (advectionProgram.uniforms.uVelocity) {
  ...
  return (
    <div className="pointer-events-none fixed top-0 left-0 z-50 w-full h-full">
      <canvas
        id="fluid"
        ref={canvasRef}
